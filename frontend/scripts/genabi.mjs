...
const CONTRACT_NAME = "WaterSaveLog";

// <root>/actio/contracts
const rel = "../contracts";

// <root>/actio/frontend/abi
...
if (!fs.existsSync(dir)) {
  console.error(
    `${line}Unable to locate ${rel}. Expecting <root>/actio/${dirname}${line}`
  );
  process.exit(1);
}
...
function readDeployment(chainName, chainId, contractName, optional) {
  ...
  if (!fs.existsSync(chainDeploymentDir) && chainId === 31337) {
    console.warn(
      `${line}Deployment not found for ${chainName}. Please deploy first: cd ${dirname} && npx hardhat deploy --network ${chainName}${line}`
    );
  }
